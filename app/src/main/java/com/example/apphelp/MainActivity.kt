...
class MainActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun sendSms() {
        if ((ContextCompat.checkSelfPermission(this, Manifest.permission.READ_SMS) +
            ...
            thread { // a thread is used so you can send sms every x time with the updated position
                ...
                getCurrentLocation() //to catch the current position (to send it via SMS)
                ...
                for(k in 0..5) {
                    ...
                    Thread.sleep(60000)
                } //6 messages are sent to each emergency contact, one message per minute
            }
        }
    }
    ...
    private fun askPermissionSms() {
        askPermission(
            ...
            if(e.hasDenied()) { //if the permission isn't granted
                e.denied.forEach {
                }
